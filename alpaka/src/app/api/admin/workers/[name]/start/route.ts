/**
 * Admin API: Start Worker
 * Start a specific PM2 worker
 */
...
export async function POST(
  ...
  try {
    ...
    console.log(`🚀 [Workers API] Starting worker: ${name}`);

    // Start worker through PM2 (dynamic import to avoid bundling issues)
    ...
    const result = await new Promise<{ success: boolean; message: string }>((resolve, reject) => {
      pm2.connect((err) => {
        ...
        pm2.start(name, (err) => {
          ...
          if (err) {
            console.error(`❌ [Workers API] Failed to start ${name}:`, err);
            return resolve({
              success: false,
              message: err.message
            });
          ...
          console.log(`✅ [Workers API] Worker ${name} started successfully`);
          resolve({
            success: true,
            message: `Worker ${name} started successfully`
          });
        });
      });
    });
